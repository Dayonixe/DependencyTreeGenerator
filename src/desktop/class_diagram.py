...
class ClassCard(QGraphicsObject):
    ...
    def __init__(self, info: ClassInfo, colors):
        super().__init__()
        self.info = info
        self.colors = colors
        self.expanded = False
    ...
    def paint(self, painter, option, widget=None):
        ...
        painter.setBrush(QColor(
            self.colors["class_selected_header"] if selected else self.colors["class_header"]
        ))
        painter.drawRoundedRect(QRectF(1, 1, self.WIDTH - 2, self.HEADER_HEIGHT - 1), 8, 8)
        painter.setPen(QColor(self.colors["selected_text"] if selected else self.colors["class_text"]))
        ...
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QColor(self.colors["selected_text"] if selected else self.colors["internal"]))
        direction = QPolygonF([
        ...
        if self.expanded:
            painter.setPen(QPen(QColor(self.colors["class_separator"]), 1))
            painter.drawLine(QPointF(0, self.HEADER_HEIGHT), QPointF(self.WIDTH, self.HEADER_HEIGHT))
            painter.setFont(QFont("Consolas", 8))
            painter.setPen(QColor(self.colors["class_method"]))
            methods = self.info.methods or ()
            if not methods:
                painter.setPen(QColor(self.colors["card_muted"]))
...
class InheritanceEdge(QGraphicsPathItem):
    ...
    def __init__(self, parent: ClassCard, child: ClassCard, colors):
        super().__init__()
    ...
    def apply_theme(self, colors):
        self.colors = colors
        self.setPen(QPen(QColor(colors["inheritance_edge"]), 1.7))
        self.update()
    ...
    def paint(self, painter, option, widget=None):
        ...
        painter.setBrush(QColor(self.colors["canvas"]))
...
class UsageEdge(QGraphicsPathItem):
    ...
    def __init__(self, source: ClassCard, target: ClassCard, usages: tuple[ClassUsage, ...], colors):
        super().__init__()
    ...
    def apply_theme(self, colors):
        self.colors = colors
        pen = QPen(QColor(colors["call_edge"]), 1.9, Qt.PenStyle.DashLine)
...
class ClassDiagramView(QGraphicsView):
    ...
    def set_classes(self, classes, usages=(), query="", *, reset=False):
        ...
        with QSignalBlocker(self.scene()):
            ...
            for info in visible:
                ...
                card = ClassCard(info, self.colors)
            ...
            for info in visible:
                ...
                for base in info.bases:
                    ...
                    if parent is not None:
                        edge = InheritanceEdge(parent, child, self.colors)
            ...
            for (source, target), items in grouped.items():
                edge = UsageEdge(source, target, tuple(items), self.colors)
                self.scene().addItem(edge)
                self.edges.append(edge)
    ...
    def apply_theme(self, name: str):
        """Repaint the diagram while preserving its interactive state."""
